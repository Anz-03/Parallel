...
#pragma acc enter data copyin(filteredImage[0 : buffer_size], \
                              input_jpeg.buffer[0 : buffer_size],        \
...
#pragma acc parallel present(                                \
    filteredImage[0 : buffer_size], input_jpeg.buffer[0 : buffer_size], \
...
#pragma acc loop independent
            for (int x = 1; x < width - 1; x++)
            {
                ...
                float r_sum = acc_linear_filter(input_jpeg.buffer, filter, r_id, width,
                                                num_channels);

                float g_sum = acc_linear_filter(input_jpeg.buffer, filter, g_id, width,
                                                num_channels);

                float b_sum = acc_linear_filter(input_jpeg.buffer, filter, b_id, width,
                                                num_channels);
